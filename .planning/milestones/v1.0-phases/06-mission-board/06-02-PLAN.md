...
must_haves:
  truths:
    ...
    - "User can toggle the sign-off gate (signOffRequired) on an existing task from the task detail metadata sidebar"
  artifacts:
...
<task type="auto">
  ...
  <action>
    ...
    **2. Create TaskDetailContent** at `src/features/missions/components/task-detail-content.tsx`:
    ...
    - Renders task data from useTask(taskId) hook:
      ...
      - **Sign-off gate toggle** (TASK-06): In the metadata sidebar (right column when isCompact=false, or inline section when isCompact=true), render a Switch component with label "Require sign-off". Bound to task.signOffRequired. On toggle, call useUpdateTask mutation with `{ signOffRequired: !current }` for optimistic update. Show brief helper text: "When enabled, task requires human approval before moving to DONE." This allows toggling signOffRequired on existing tasks, not just at creation time.
    ...
    **4. Wire slide-over into KanbanBoard/MissionsBoardView:**
    ...
    - **Click-vs-drag detection spec:** Use a `useRef<boolean>` named `wasDraggingRef` in MissionsBoardView (the component that owns both the DndContext and the TaskSlideOver).
      - In KanbanBoard's `onDragStart` handler: set `wasDraggingRef.current = true`
      - In KanbanBoard's `onDragEnd` handler: clear with `setTimeout(() => { wasDraggingRef.current = false }, 200)` (200ms debounce allows the click event to fire and be checked before the flag resets)
      - Pass `wasDraggingRef` to KanbanCard via React context (create a `DragStateContext = createContext<React.RefObject<boolean>>()` in the board module) or as a prop through KanbanColumn
      - In KanbanCard's `onClick` handler: check `if (wasDraggingRef.current) return;` before calling `setSelectedTask(task.id)`
      - This ensures clicks during/immediately-after drag are suppressed, while normal clicks open the slide-over
  ...
  <done>
    Clicking a Kanban card opens a right-side slide-over with task details (click-vs-drag distinguished via wasDraggingRef with 200ms timeout). Slide-over shows title, priority, status, agents, description, tags, subtasks, sign-off toggle, and expand button. Sign-off toggle in metadata sidebar allows toggling signOffRequired on existing tasks with optimistic update. Full page at /missions/[taskId] shows two-column layout with all fields. Delete works with confirmation dialog. Placeholder sections ready for 06-03 (timeline, deliverables) and 06-04 (comments). Build passes.
  </done>
</task>
...
10. Sign-off toggle in task detail metadata sidebar toggles signOffRequired with optimistic update
</verification>
...
- Sign-off gate toggle on existing tasks via Switch in metadata sidebar, wired to useUpdateTask
