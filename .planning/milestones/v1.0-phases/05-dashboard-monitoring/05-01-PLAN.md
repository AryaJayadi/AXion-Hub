...
must_haves:
  truths:
    - "Dashboard data layer is operational — widgets can subscribe to live agent/cost/activity data via Zustand stores wired to EventBus"
    - "Activity events are buffered so the dashboard feed always shows the most recent 20 events without unbounded memory growth"
    - "BentoGrid renders responsive single-column on mobile and 4-column on lg breakpoint"
    - "Cost formatting utilities produce human-readable token counts (K/M suffixes) and dollar amounts for display"
  artifacts:
