...
must_haves:
  ...
  key_links:
    ...
    - from: "src/widgets/app-shell/components/header-bar.tsx"
      to: "src/features/dashboard/components/notification-bell.tsx"
      via: "NotificationBell rendered in HeaderBar between search and UserMenu"
...
<task type="auto">
  ...
  <action>
    ...
    5. Mount NotificationBell in HeaderBar (`src/widgets/app-shell/components/header-bar.tsx`):
       - Import `NotificationBell` from `@/features/dashboard/components/notification-bell`
       - Render `<NotificationBell />` between the search button and the `<UserMenu>` component
       - This ensures users see the in-app bell notification across all pages

    6. Create `src/views/dashboard/monitor-alerts-view.tsx`:
       - "use client" component
    ...
    7. Create `app/(dashboard)/monitor/alerts/page.tsx`:
       - Import and render `MonitorAlertsView`
       - Export metadata: `{ title: "Alert Rules | AXion Hub" }`
  </action>
  <verify>
    Run `bun run build` -- zero errors. Verify /monitor/alerts route exists. Verify AlertTemplatePicker renders 6 templates. Verify AlertRuleForm uses react-hook-form with zodResolver. Verify NotificationBell reads from alert store. Verify header-bar.tsx imports and renders NotificationBell between search button and UserMenu.
  </verify>
  <done>
    /monitor/alerts page renders a DataTable of alert rules with inline toggle switches, create/edit/delete functionality, and template-based rule creation. Notification bell is mounted in the global HeaderBar between the search button and UserMenu, visible across all pages. Alert rule form validates with Zod and supports webhook URL configuration. The full alert management experience is functional with mock data.
  </done>
</task>
