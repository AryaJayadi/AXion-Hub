...
must_haves:
  truths:
    ...
    - "User sees token cost summary with time toggle and per-agent stacked bar chart"
  ...
  key_links:
    ...
    - from: "src/features/dashboard/components/cost-summary-widget.tsx"
      to: "src/features/dashboard/model/dashboard-store.ts"
      via: "useDashboardStore selector for costByPeriod"
      pattern: "useDashboardStore"
...
<task type="auto">
  ...
  <action>
    ...
    5. Create `cost-summary-widget.tsx`:
       ...
       - Bottom section: stacked BarChart (input vs output tokens per agent) using ChartContainer with `min-h-[200px]`
