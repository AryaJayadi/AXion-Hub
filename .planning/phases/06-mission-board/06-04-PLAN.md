...
must_haves:
  truths:
    ...
    - "Posting a comment with @mentions emits a 'task.comment.added' EventBus event with mentions payload for notification handling"
  ...
  key_links:
    ...
    - from: "src/features/missions/components/task-comments.tsx"
      ...
      via: "Comment submit emits task.comment.added event with mentions for notifications"
      pattern: "eventBus\\.emit.*task\\.comment\\.added"
...
<task type="auto">
  ...
  <action>
    ...
    **2. Create TaskComments** at `src/features/missions/components/task-comments.tsx`:
    ...
    - **Comment input:**
      ...
      - On submit: create mock comment with nanoid, add to local comments state, show toast "Comment added". **Also emit EventBus event:** `eventBus.emit('task.comment.added', { taskId, authorId: 'current-user', authorType: 'user', content, mentions })` where `mentions` is the array of Mention objects parsed from the submitted content. Import eventBus from the gateway-connection module. This enables the subscription in mission-subscriptions.ts to react (e.g., for future notification handling) and satisfies TASK-07's "trigger notifications" requirement per ROADMAP SC4.
  ...
  <done>
    Comment thread renders with author avatars, timestamps, and highlighted @mentions. Typing @ opens mention popover with searchable agent/human list using cmdk. Selected mentions insert as formatted text. Comments post with optimistic update and emit eventBus 'task.comment.added' event with mentions payload for notification handling. Wired into TaskDetailContent in both slide-over and full page views. Build passes.
  </done>
</task>
