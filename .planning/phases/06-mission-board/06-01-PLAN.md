...
**Column naming note:** REQUIREMENTS.md (TASK-01) references "ASSIGNED" and "REVIEW" column names. Per CONTEXT.md locked decision, these are intentionally renamed to "QUEUED" and "IN REVIEW" for the 6-column pipeline (INBOX, QUEUED, IN PROGRESS, IN REVIEW, DONE, ARCHIVED). The semantic meaning is preserved; naming aligns with the user's confirmed board design.
</objective>
...
<task type="auto">
  <name>Task 1: Install dnd-kit + immer, create mission entity types, schemas, utils, and shared extensions</name>
  <files>
    package.json
    src/entities/mission/model/types.ts
    src/entities/mission/model/schemas.ts
  ...
  <action>
    ...
    **7. Add mission events to KnownEvents** in `src/features/gateway-connection/lib/event-bus.ts`:
    - Add these typed events:
      ...
      - `'task.comment.added': { taskId: string; authorId: string; authorType: 'agent' | 'user'; content: string; mentions: Array<{ type: 'agent' | 'human'; id: string; name: string }> }`

    **8. Extend query keys** in `src/shared/lib/query-keys.ts`:
  ...
  <verify>
    Run `bun run build` -- must pass with zero errors. Verify @dnd-kit/core, @dnd-kit/sortable, @dnd-kit/utilities, @dnd-kit/modifiers, and immer appear in package.json dependencies. Verify entity barrel exports resolve correctly.
  </verify>
  <done>
    Mission entity types fully defined with 11 interfaces. Zod schemas for task/board creation and validation. Priority and column utils with display mappings. EventBus extended with 5 task event types (including task.comment.added with mentions payload). Query keys extended with boards and tasks domains. Entity barrel export resolves. Build passes.
  </done>
</task>

<task type="auto">
  <name>Task 2: Board store, task store, hooks, TanStack Query hooks with mock data, subscriptions, and drag-utils</name>
  <files>
    src/features/missions/model/board-store.ts
    src/features/missions/model/task-store.ts
    src/features/missions/model/hooks.ts
  ...
  <action>
    **1. Create board store** at `src/features/missions/model/board-store.ts`:
    ...
    **2. Create task store** at `src/features/missions/model/task-store.ts`:
    ...
    **3. Create hooks** at `src/features/missions/model/hooks.ts`:
    ...
    **4. Create TanStack Query hooks** at `src/features/missions/api/use-boards.ts` and `use-board-tasks.ts`:
    ...
    **5. Create EventBus subscriptions** at `src/features/missions/lib/mission-subscriptions.ts`:
    ...
    **6. Create drag-utils** at `src/features/missions/lib/drag-utils.ts`:
    ...
    **7. Create barrel export** at `src/features/missions/index.ts` -- export stores, hooks, types, subscriptions
  </action>
  <verify>
    Run `bun run build` -- must pass with zero errors. Verify feature barrel exports resolve correctly. Verify board store, task store, and hooks are importable.
  </verify>
  <done>
    Board store with immer supports moveCard, reorderInColumn, autoTransition with pending transition queue. Task store holds Map of tasks. Selector hooks for board state and task data. TanStack Query hooks with mock data provide realistic board state (8-10 tasks across 6 columns). EventBus subscriptions wired for agent events. Drag-utils for column lookup. Feature barrel export resolves. Build passes.
  </done>
</task>

<task type="auto">
  <name>Task 3: Kanban board UI with drag-and-drop, card components, and /missions route page (depends on Tasks 1-2)</name>
  <files>
    src/features/missions/components/kanban-board.tsx
    src/features/missions/components/kanban-column.tsx
    src/features/missions/components/kanban-card.tsx
