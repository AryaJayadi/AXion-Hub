...
<task type="auto">
  ...
  <action>
    ...
    **use-workflows.ts**: `useWorkflows()` returning 5-6 mock WorkflowDefinition objects with varied statuses, trigger types, and node counts. Include `useCreateWorkflow` and `useDeleteWorkflow` mutations. Add `workflows` domain to query-keys.ts (NOTE: read current file state before adding workflows domain to avoid overwriting skills/plugins additions from Plans 01/02 which also modify this file in Wave 1):
    ```
    workflows: {
      all: ["workflows"],
