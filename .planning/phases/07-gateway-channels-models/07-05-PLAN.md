...
<task type="auto">
  ...
  <action>
    ...
    **Failover chain builder** (`failover-chain-builder.tsx`):
    ...
    - "Add Model" button at bottom of the chain: opens a dropdown/popover showing all available models from all providers (grouped by provider). Use `useProviders()` to populate the Add Model popover, mapping each provider's `models` array into grouped options by provider name. Selecting a model adds it to the chain with default retries (2) and timeout (30000ms).
