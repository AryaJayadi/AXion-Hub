...
<task type="auto">
  ...
  <action>
    ...
    **2. Update app-sidebar.tsx**:
    ...
    Note: The `UserMenu` was integrated into `SidebarFooter` in Plan 03. If it's currently in the header bar, keep it there and also add a condensed version in the footer showing just the avatar (following shadcn sidebar-07 pattern). The key requirement is that the org switcher goes in `SidebarHeader`.
  </action>
  <verify>
    - `bun run build` passes
