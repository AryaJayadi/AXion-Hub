...
must_haves:
  truths:
    ...
    - "If a direct conversation already exists with the selected agent, it is resumed instead of creating a new one"
  artifacts:
...
<task type="auto">
  ...
  <action>
Four changes to wire the "New Chat" and "Start Chat" buttons and implement "resume existing conversation" per locked decision:
...
**4. Resume existing conversation instead of always creating new (chat-layout.tsx):**
Per locked decision: "After picking an agent, resume existing conversation if one exists (navigate to /chat/[agentId])."

Modify `handleCreateConversation` in chat-layout.tsx to check for an existing conversation before creating a new one. Also add `router.push` for navigation after selection:
...
const handleCreateConversation = useCallback(
    (agentIds: string[], title?: string) => {
        ...
        if (agentIds.length === 1) {
            const existingConvo = Array.from(store.conversations.values()).find(
                (c) => c.type === "direct" && c.agentIds.length === 1 && c.agentIds[0] === agentIds[0]
            );
            if (existingConvo) {
...
Key behaviors:
- For single-agent (direct) chat: searches conversations Map for an existing direct conversation with that agentId. If found, sets it active and navigates.
- For multi-agent (room) chat: always creates a new room (no resume logic for rooms).
- Both paths navigate to `/chat/[agentId]` after selection.
  </action>
  <verify>
...
7. handleCreateConversation checks for existing direct conversation before creating new
8. Both new and resumed conversations navigate to /chat/[agentId] via router.push
</verification>
...
- Selecting an agent resumes existing direct conversation if one exists, otherwise creates new
- Agent selection always navigates to /chat/[agentId]
</success_criteria>
